# StarCrew: Starships CaRgo Exploration Workflow

## Overview

**StarCrew** is a bash pipeline specifically designed to systematically analyze the cargo genes of Starship elements. StarCrew is composed of seven distinct commands and creates a well-organized project directory to facilitate downstream analysis. StarCrew provides an initial analysis workflow, ranging from captain identification (to confirm upstream analysis) to the functional annotation of orthogroups within the cargo genes. The primary goal of this workflow is to offer researchers a simple, integrated workflow for an initial exploratory analysis and comparison of Starship cargo genes. This process is expected to help identify biological patterns or generate hypotheses that can be further tested either by bioinformatic or wet-lab experimental approaches.
...
Due to the size of the functional annotation databases, I recommended checking your available storage before installing StarCrew. It is recommended to have between 100 - 200 GB of free disk space, depending on your intended dataset.

> [!NOTE]
> If free disk space is an issue, StarCrew can be configured without downloading the annotation databases. However, please note that this will leave the `OrthogroupsAnnotation` command unusable, as it depends entirely on these local resources.

## Installation

Before installation, ensure you have `Anaconda3` and `git` installed and accessible in your system path, as they are required for the full installation and set-up. After cloning, execute the `build_StarCrew.sh` script to properly configure a conda environment, install all necessary software dependencies and download the databases required by the workflow's commands, as shown below:
...
2.  **Gene Prediction File (GFF3):** A GFF3 file containing the gene predictions for all elements listed in the `Elements` fasta file.
    ...
    - **Tip:** You can obtain an updated GFF file from the original genome GFF by using the auxiliary script `gff_slicer.py`.
...
4.  **Gene Prediction Path File (TSV):**
    ...
    - **Crucial Points:**
        ...
        - The GFF path must point to the original GFF file, and not the files returned by the `starfish format` or `starfish format-ncbi` commands. StarCrew requires the gene and CDS information, and needs the original contig `seqID` to be maintained. The files returned by the `starfish format` or `starfish format-ncbi` commands only retain mRNA information and alter the IDs to suit the Starfish workflow.
...
Optional args:
-M, --Metadata: csv file delimited by semicolon with the metadata information (Check documentation for more information).
...
3. **Gene Content Filtering:** This filter removes elements that are empty or contain a very low number of cargo genes.
    - Since the primary goal of StarCrew is to analyze cargo content, maintaining elements without sufficient cargo genes is inefficient. This filter ensures that only elements useful for downstream analysis are retained.
...
Usage: StarCrew GeneHomogenization [-help] -w <directory_path> [ -t <integer> --overwrite ]
...
Executes six main steps:
  ...
  4. Collinearity summarization. Available modes:
     ...
     Constellation- Returns pairs that have been filtered using a BLAST-based approach at the nucleotide level or a GCP threshold.
  5. Spectral clustering to define element sub-clusters.
  6. Per-cluster data organization.

Syntax: StarCrew SyntenyClustering [-help] -w <directory_path> [ -m <string> -a <integer> -g <integer> -e <float> -n <integer> -s <integer> -th <float> { -fs <integer> -ms <integer> -i <float> -c <float> | -cm <string> -p <float>} -t <integer> --preCluster --captainInfo { --overwrite | --skip-syntenet } ]

Required args:
-w, --workingDirectory  Working directory where all data are stored.

Required args with defaults:
-m, --mode              Summarization mode (Default: FilterMetric) [Available: Raw, SSP, FilterBlast, FilterMetric, Constellation].
...
Required args with defaults in 'Constellation' mode:
-cm, --classMode        Constellation mode (Default: Blastn) [Available: Blastn, GCP] Note: In Blastn mode, the -fs,-ms, and -i flags are used too.
-p, --percentage        Percentage of coverage (Default: 80.0) [range: 70.0 - 90.0]"
...
--overwrite             Overwrite a previous run (Default: off).
--skip-syntenet         Skip collinearity detection to re-run with different mode or clustering parameters. Not compatible with '--overwrite' (Default: off).
-help                   Display this help message.
```
...
5. **Constellation:**
    - This filtering approach is designed to guide Constellation definition and it support two modes:
...
2.  **Cluster Definition:** A cluster is defined as a set of connected elements. **Note:** Due to the nature of connected components, not all elements inside a cluster may have a direct connection between them, and therefore might not share any sequence or cargo similarity.
    - **2.1. Constellation cluster:** In constellation mode, an additional check step is performed to identify articulation points within the network. If any articulation points are found, the cluster is broken down accordingly.
...
Syntax: StarCrew ClusterCharacterization [-help] -w <directory_path> [ -l <integer> -i <float> -t <integer> --overwrite --constellation ]
...
Optional args:
  ...
  -c, --constellation  Avoid Captain infromation and analyze all available cluster
                       [The clusters are expected to come from the 'Constellation'
                       mode in SyntenyClustering].
  --overwrite     Overwrite a previous run (Default: off).
  --skip-orthofinder     Skip OrthoFinder and re-run from blastn step.
...
The main goal of this command is to identify three relevant groups of genes within each cluster:

- **Core Genes:**
    - We use a loose definition of "core gene" in this context. While core genes are generally defined as genes present in all genomes of a dataset, or in a less strict definition (often called soft core) as genes present in at least 95%-98% of genomes, this strict threshold is inappropriate here. This is due to the inherent variability of cargo genes across different element haplotypes and the nature of the cluster definition in StarCrew.
...
- **SubCluster Accessory Genes:**
    - Using the same threshold as for Core genes, these are genes identified as common within a specific subcluster
- **Shared Accessory Genes:**
    - Accessory genes that are shared between multiple subclusters but do not meet the threshold to be considered core genes.z
...
Syntax: StarCrew OrthogroupsOverrepresentation [-help] -w <directory_path> [ -m <string> { -a <string> -c <float> -cm <string> | -n <string> -v <string> -p <float> -pa <string> } -t <integer> { --overwrite | --skip-orthofinder } ]
...
- **metadata_files/**: This directory stores metadata information. Two files are found here:
...
As previously mentioned, this Workflow is composed of a series of sequential commands. It can perform two distinct types of analysis depending on the research objectives: Cargo Gene Dynamics and Enrichment Analysis. 
...
Lizcano-Salas, A.F.; Ryberg, M.; Vogan, A.A. (2026). StarCrew: A pipeline for exploratory analysis of Starship cargo allowed the identification of a conserved pseudokinase [Manuscript in preparation].

StarCrew is a Workflow that calls different bioinformatic software, for that reason any publication of results obtained by StarCrew required the citation of the tools that were called.
