...
def find_clusters(pairs):
    """Finds weakly connected components (clusters) from a list of edges."""
...
def find_sub_clusters_spectral(all_raw_weighted_edges, main_clusters, 
    ...
    """
    Applies a modularity-based pre-analysis followed by adaptive spectral clustering.
    Ensures all final subclusters are formally connected components.
    """
    ...
    for original_cluster_idx, main_cluster_nodes in enumerate(main_clusters):
        ...
        start_k = max(2, mod_k - 2)
        k = start_k
        
        print(f"Cluster {original_cluster_idx}: Modularity found {mod_k} groups. Starting Spectral at K={k}")
        ...
        while True:
            ...
            try:
                sc = SpectralClustering(n_clusters=k, affinity='precomputed', n_init=10, assign_labels='kmeans')
        ...
        if best_partition:
            # --- NEW STEP: Ensure connectivity within proposed spectral clusters ---
            # Spectral clustering doesn't guarantee that nodes in the same label are connected.
            connected_partition = []
            for part_nodes in best_partition:
                part_subgraph = subgraph.subgraph(part_nodes)
                # Split the label into its actual connected components
            ...
            while True:
                ...
                for target_id, target_nodes in current_communities.items():
                    if target_id == to_merge_id: continue
                    conn_weight = 0
                ...
                if best_target is not None and max_weight > 0:
                    ...
                    # If it's small but has NO connection to other sub-clusters (isolated component),
                    # we keep it or handle it as its own entity depending on needs. 
                    # Here we stop merging to avoid forcing disconnected merges.
                    break
...
def write_clusters_to_file(filename, clusters_data, main_cluster_id_map=None):
    """Writes clusters to file. Returns a map if writing main clusters."""
...
def write_cytoscape_node_attributes(filename, sub_info, m_map, node_meta, meta_keys):
    ...
    with open(filename, 'w') as f:
        ...
        for nid in sorted(node_attrs.keys()):
            a = node_attrs[nid]
            row = [nid, a['SubID'], a['MainID'], str(a['Split'])]
...
def write_cluster_stats(filename, main_c, sub_info, m_map):
    ...
    for p_idx, _, was_split in sub_info:
        if was_split: sub_counts[m_map[p_idx]] += 1
    ...
    try:
        with open(input_file, 'r') as f:
            ...
            try:
                ...
                start = 1
                for i in range(5, len(first), 2):
                    m1 = re.match(r"element01_(.*)", first[i])
                    m2 = re.match(r"element02_(.*)", first[i+1]) if i+1 < len(first) else None
            ...
            for line in lines[start:]:
                ...
                try:
                    ...
                    pairs.append((n1, n2))
                    if meta_keys:
                        if n1 not in node_meta: node_meta[n1] = {k: p[meta_map[k][0]] for k in meta_keys if meta_map[k][0] < len(p)}
                        if n2 not in node_meta: node_meta[n2] = {k: p[meta_map[k][1]] for k in meta_keys if meta_map[k][1] < len(p)}
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--min-nodes', type=int, default=4)
